...
public class DistributedHBaseCluster extends HBaseCluster {
  ...
  private org.apache.hadoop.hbase.ClusterManager clusterManager;
  /**
  ...
  public DistributedHBaseCluster(Configuration conf, org.apache.hadoop.hbase.ClusterManager clusterManager)
      throws IOException {
    super(conf);
  ...
  public void startRegionServer(String hostname, int port) throws IOException {
    ...
    clusterManager.start(ClusterManager.ServiceType.HBASE_REGIONSERVER, hostname, port);
  }
  ...
  public void killRegionServer(ServerName serverName) throws IOException {
    ...
    clusterManager.kill(ClusterManager.ServiceType.HBASE_REGIONSERVER,
  ...
  public void stopRegionServer(ServerName serverName) throws IOException {
    ...
    clusterManager.stop(ClusterManager.ServiceType.HBASE_REGIONSERVER,
      serverName.getHostname(), serverName.getPort());
  }

  @Override
  public void waitForRegionServerToStop(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStop(ClusterManager.ServiceType.HBASE_REGIONSERVER, serverName, timeout);
  }

  @Override
  public void suspendRegionServer(ServerName serverName) throws IOException {
    LOG.info("Suspend RS: {}", serverName.getServerName());
    clusterManager.suspend(ClusterManager.ServiceType.HBASE_REGIONSERVER,
        serverName.getHostname(), serverName.getPort());
  }
  ...
  public void resumeRegionServer(ServerName serverName) throws IOException {
    ...
    clusterManager.resume(ClusterManager.ServiceType.HBASE_REGIONSERVER,
        serverName.getHostname(), serverName.getPort());
  }
  ...
  public void startZkNode(String hostname, int port) throws IOException {
    ...
    clusterManager.start(ClusterManager.ServiceType.ZOOKEEPER_SERVER, hostname, port);
  }
  ...
  public void killZkNode(ServerName serverName) throws IOException {
    ...
    clusterManager.kill(ClusterManager.ServiceType.ZOOKEEPER_SERVER,
      serverName.getHostname(), serverName.getPort());
  }
  ...
  public void stopZkNode(ServerName serverName) throws IOException {
    ...
    clusterManager.stop(ClusterManager.ServiceType.ZOOKEEPER_SERVER,
      serverName.getHostname(), serverName.getPort());
  }

  @Override
  public void waitForZkNodeToStart(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStart(ClusterManager.ServiceType.ZOOKEEPER_SERVER, serverName, timeout);
  }

  @Override
  public void waitForZkNodeToStop(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStop(ClusterManager.ServiceType.ZOOKEEPER_SERVER, serverName, timeout);
  }
  ...
  public void startDataNode(ServerName serverName) throws IOException {
    ...
    clusterManager.start(ClusterManager.ServiceType.HADOOP_DATANODE,
      serverName.getHostname(), serverName.getPort());
  }
  ...
  public void killDataNode(ServerName serverName) throws IOException {
    ...
    clusterManager.kill(ClusterManager.ServiceType.HADOOP_DATANODE,
      serverName.getHostname(), serverName.getPort());
  }
  ...
  public void stopDataNode(ServerName serverName) throws IOException {
    ...
    clusterManager.stop(ClusterManager.ServiceType.HADOOP_DATANODE,
      serverName.getHostname(), serverName.getPort());
  }

  @Override
  public void waitForDataNodeToStart(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStart(ClusterManager.ServiceType.HADOOP_DATANODE, serverName, timeout);
  }

  @Override
  public void waitForDataNodeToStop(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStop(ClusterManager.ServiceType.HADOOP_DATANODE, serverName, timeout);
  }
  ...
  public void startNameNode(ServerName serverName) throws IOException {
    ...
    clusterManager.start(ClusterManager.ServiceType.HADOOP_NAMENODE, serverName.getHostname(),
      serverName.getPort());
  }
  ...
  public void killNameNode(ServerName serverName) throws IOException {
    ...
    clusterManager.kill(ClusterManager.ServiceType.HADOOP_NAMENODE, serverName.getHostname(),
      serverName.getPort());
  }
  ...
  public void stopNameNode(ServerName serverName) throws IOException {
    ...
    clusterManager.stop(ClusterManager.ServiceType.HADOOP_NAMENODE, serverName.getHostname(),
      serverName.getPort());
  }

  @Override
  public void waitForNameNodeToStart(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStart(ClusterManager.ServiceType.HADOOP_NAMENODE, serverName, timeout);
  }

  @Override
  public void waitForNameNodeToStop(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStop(ClusterManager.ServiceType.HADOOP_NAMENODE, serverName, timeout);
  }

  private void waitForServiceToStop(ClusterManager.ServiceType service, ServerName serverName, long timeout)
    throws IOException {
  ...
  private void waitForServiceToStart(ClusterManager.ServiceType service, ServerName serverName, long timeout)
    throws IOException {
  ...
  public void startMaster(String hostname, int port) throws IOException {
    ...
    clusterManager.start(ClusterManager.ServiceType.HBASE_MASTER, hostname, port);
  }

  @Override
  public void killMaster(ServerName serverName) throws IOException {
    LOG.info("Aborting Master: {}", serverName.getServerName());
    clusterManager.kill(ClusterManager.ServiceType.HBASE_MASTER, serverName.getHostname(), serverName.getPort());
  }

  @Override
  public void stopMaster(ServerName serverName) throws IOException {
    LOG.info("Stopping Master: {}", serverName.getServerName());
    clusterManager.stop(ClusterManager.ServiceType.HBASE_MASTER, serverName.getHostname(), serverName.getPort());
  }

  @Override
  public void waitForMasterToStop(ServerName serverName, long timeout) throws IOException {
    waitForServiceToStop(ClusterManager.ServiceType.HBASE_MASTER, serverName, timeout);
  }
  ...
  protected boolean restoreMasters(ClusterMetrics initial, ClusterMetrics current) {
    ...
    if (!ServerName.isSameAddress(initMaster, current.getMasterName())) {
      ...
      try {
        if (!clusterManager.isRunning(ClusterManager.ServiceType.HBASE_MASTER,
      ...
      for (ServerName sn:toStart) {
        try {
          if(!clusterManager.isRunning(ClusterManager.ServiceType.HBASE_MASTER, sn.getHostname(), sn.getPort())) {
  ...
  protected boolean restoreRegionServers(ClusterMetrics initial, ClusterMetrics current) {
    ...
    for(ServerName sn:toStart) {
      try {
        if (!clusterManager.isRunning(ClusterManager.ServiceType.HBASE_REGIONSERVER, sn.getHostname(),
